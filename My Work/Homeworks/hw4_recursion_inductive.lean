...
| zero zero := ff
| zero n := tt
| (succ n') zero := ff -- syntax is much easier than you're making it
...
def mzero := zero --0
def mone := succ zero -- 1
def mtwo := succ (succ zero) -- 2
def mthree := succ (succ (succ zero)) --3
def mfour := succ(succ(succ(succ zero)))

#reduce lt_mnat mzero mzero -- 0 0
#reduce lt_mnat mzero mtwo -- 0 2
#reduce lt_mnat mtwo mtwo -- 2 2
#reduce lt_mnat mtwo zero -- 2 0
#reduce lt_mnat mtwo mthree -- 2 3
#reduce lt_mnat mthree mtwo -- 3 2
...
| zero := succ zero
| (succ n') := mult_mnat (succ n') (fac_mnat n') -- essentially I'm trying to say (succ n') * (n'!)

/-
Factorial function breakdown:
n! = n * (n-1) * (n-2) *...* 1
(n+1)! = (n+1) * n!
-/
...
| nil := tt
| _ := ff -- see what's wrong with this
...
| nil nil := nil
| (empty_list) (cons h t) := cons h t
| (cons h t) (empty_list) := cons h t
| (cons h t) (cons l m) := cons(h (append_mnat (append_mnat t l) m)) -- come back to this
...
right answers.
-/ 
def lnone: list_mnat := nil
def ltest: list_mnat := nil
def l1: list_mnat :=
                (cons
                    mone
                    (cons
...
#reduce append_mnat lnone ltest
#reduce append_mnat lnone l1
#reduce append_mnat l1 lnone
#reduce append_mnat l1 l2
end my_list_mnat
